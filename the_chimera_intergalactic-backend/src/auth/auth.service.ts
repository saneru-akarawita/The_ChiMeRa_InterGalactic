...
export class AuthService {
    ...
    async signUpTraveler(createTravelerDto: CreateTravelerDto) {
        ...
        const createdTraveler = await this.usersService.createTraveler(createTravelerDto)
    ...
    async signInTraveler({ email, password }: TravelerLoginDto) {
        ...
        if (!traveler) throw new BadRequestException({
            errorMessages: [{ property: 'system', errors: ['Something went wrong'] }],
            message: 'Bad Request',
            statusCode: 400,
        });
        ...
        if (!passwordMatches)
            throw new BadRequestException({
                errorMessages: [{ property: 'pasword', errors: ['Password does not match'] }],
                message: 'Bad Request',
                statusCode: 400,
            });
